...
void RSA_generate_keyring(){
    ...
    // Generates RSA both public and private keys
    ...
    FILE *pub_file = fopen("public_key.key", "wb");
...
void RSA_encrypt() {
    ...
    FILE *pubf = fopen("./keys/public_key.key", "r");
    if (!pubf) {
    ...
    FILE *aes_key_file = fopen("./keys/AES.key", "rb");
    if (!aes_key_file) {
    ...
    // Optionally save the encrypted AES key
    FILE *outf = fopen("./keys/AES.key", "wb");
    if (outf) {
...
void RSA_decrypt() {
    ...
    FILE *privf = fopen("./keys/private_key.key", "r");
    if (!privf) {
    ...
    FILE *encf = fopen("./keys/AES.key", "rb");
    if (!encf) {
    ...
    // Determine decrypted output length
    size_t outlen;
    ...
    decrypted[outlen] = '\0'; // Optional, null-terminate if it's printable

    // Print or save the decrypted AES key
    ...
    if (outf) {
        ...
        fclose(outf);
        printf("Decrypted AES key saved to ./keys/AES.key.dec\n");
    }
